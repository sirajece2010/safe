...
def tshark_wrapper(child, cmd, count=2, interface="wlp2s0", time=30):
    global _pcap_name
...
def calc_psid_len(data):
    if int(data[0:2], 16) == 128:
        ...
        return 4
    elif int(data[0:6], 16) == '818181':
        return int(data[6:8], 16)
    else:
        return -1


def parse_pcaps(pcap_file, psid, service_type=None):
    data_dict = {}
    for pkt in pcap_file:
        hex_resp = pkt.data.data
        if pkt.ip.proto != 17 and hex_resp[22:24] != '82':
            continue
        if service_type == "add_user":
            psid_len = calc_psid_len(hex_resp[40:]) * 2
        else:
            psid_len = calc_psid_len(hex_resp[36:]) * 2
        if psid_len == 8:
            psid_base_index = 34 + psid_len
        elif service_type=="add_user":
            psid_base_index = 40 + psid_len
        else:
...
def pcap_reader_verify_psid(child, cmd, psid1=None, psid2=None, option=None, service_type=None):
    global _pcap_name
    sleep(3)
    ...
    if psid1 and psid2 is not None:
        ...
        packet, dict1 = parse_pcaps(pcap_file, psid1, service_type=service_type)
        print(dict1)
        print(f"evaluating for {psid2} PSID\n")
        packet, dict2 = parse_pcaps(pcap_file, psid2, service_type=service_type)
        print(dict2)
...
def coc_script(child, cmd, key, file_name, tcid, request):
    ...
    if re.search(r"stop_[\w]+_[tr]x\d?", request, re.I):
        coc_dev_state_dict[key]["stop_flag"] = 0
